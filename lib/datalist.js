...
export async function getListMain() {
    ...
    // find the URL 
    return jDataMain.map(item => {
        // use the post_date fromthe json data
        const [dateBreakdown] = item.post_date.split(" ");
        const [year, month, day] = dateBreakdown.split("-");

        
        const postUrl = `https://dev-kdurkin-sql.pantheonsite.io/${year}/${month}/${day}/${item.post_name}/`;
